...
from PyQt5.QtWidgets import QFileDialog, QWidget, QPushButton, QLabel, QComboBox, QMessageBox
...
class EncodeWindow(QWidget):
    ...
    # 加密方法
    def rsaEncode(self):
        if not self.import_address.text():
            msg = QMessageBox()
            msg.setWindowTitle('提示')
            msg.setText('请先导入公钥')
            msg.exec_()
